...
//##############Frame Capture#############///
  ...
  std::cout<<"completed setup GUI"<<std::endl;
}
...
//    if(socket->GetReceiveType()==socket->GET_POSITION)
//          {for(int i = 0;i<6;i++) {current_positions.at(i)=socket->Pulse2Joint(*(socket->GetCurrentPulse()+i),i)*M_PI/180;};
...
//  memcpy(socket->tx_file_buffer->data(),a.data(),sizeof(a));
   socket->JobFile2ByteArray("/home/haininh/my_catkin8/src/motorosudp/NINH_DEMO4.JBI");
  char jobname[8] = {'N','H','N','4','.','J','B','I'};
